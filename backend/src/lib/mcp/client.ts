...
function encryptionSecret(): string {
    const secret = process.env.MCP_CONNECTORS_ENCRYPTION_SECRET;
    if (!secret) {
        throw new Error(
            "MCP_CONNECTORS_ENCRYPTION_SECRET is not configured",
        );
    }
    return secret;
}
...
export function openaiToolName(connector: ConnectorRow, toolName: string) {
    ...
    const toolSlug = sanitizeToolPart(toolName, "tool", 24);
    ...
    return `mcp_${connectorSlug}_${toolSlug}_${toolHash}_${idSlug}`;
}
...
export function toolRequiresConfirmation(
    ...
    return (
        ...
        normalizedName === "pp_api_request" ||
        /(^|[_-])(post|put|patch|delete|create|update|upsert|send|write|remove)/.test(
            normalizedName,
        )
    );
}

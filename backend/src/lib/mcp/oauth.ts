...
function oauthClientEnvFor(serverUrl: string) {
  ...
  const isBoxMcp = hostname === "mcp.box.com";
  const prefix = isBoxMcp
    ? "BOX_MCP_OAUTH"
  ...
  return {
    clientId:
      process.env[`${prefix}_CLIENT_ID`] || process.env.MCP_OAUTH_CLIENT_ID,
    clientSecret:
    ...
    scope:
      ...
      (isBoxMcp ? "root_readwrite" : undefined),
  };
}
...
async function storeOAuthToken(
  ...
  const row = {
    ...
    scope:
      typeof token.scope === "string" ? token.scope : (config.scope ?? null),
...
async function refreshOAuthAccessToken(row: OAuthTokenRow, db: Db) {
  ...
  if (!refreshToken || !row.token_endpoint || !row.client_id) {
    throw new McpOAuthRequiredError(
      "OAuth reconnect is required for this MCP server.",
    );
  }
...
export async function oauthBearerToken(connector: ConnectorRow, db: Db) {
...
export class DbMcpOAuthProvider implements OAuthClientProvider {
  ...
  async clientInformation(): Promise<OAuthClientInformationMixed | undefined> {
    const token = await loadOAuthToken(await this.tokenConnectorId(), this.db);
    if (token?.client_id) {
      const clientSecret = decryptString(
  ...
  async saveClientInformation(info: OAuthClientInformationMixed) {
    const connectorId = await this.tokenConnectorId();
    const clientSecret =
  ...
  async tokens(): Promise<OAuthTokens | undefined> {
    const row = await loadOAuthToken(await this.tokenConnectorId(), this.db);
  ...
  async saveTokens(tokens: OAuthTokens) {
    const connectorId = await this.tokenConnectorId();
    const existing = await loadOAuthToken(connectorId, this.db);
    const existingRefresh = existing
      ? decryptString(
          existing.encrypted_refresh_token,
          existing.refresh_token_iv,
  ...
  async invalidateCredentials(
    ...
    if (scope === "tokens" || scope === "all") {
      await this.db
        .from("user_mcp_oauth_tokens")
        .delete()
        .eq("connector_id", await this.tokenConnectorId());
    }
  }
}

...
For example, sample, template, exemplar, and new-document drafting requests, follow the system's Docket Exemplar Library first rule (Box folder 404340697581 and descendants). Project documents supply the matter's facts; they do not displace the library as the first exemplar source unless the user explicitly selects a source document. Read and assess candidates before using them, and disclose any fallback to an outside-library project or matter document. If the user explicitly selects a suitable project document and wants its structure preserved, replicate it and edit the copy rather than generating a fresh document.

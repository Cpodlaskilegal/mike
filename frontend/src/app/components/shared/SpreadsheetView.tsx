    ...
    useEffect(() => {
        ...
        try {
            ...
            LuckyExcel.transformExcelToLucky(
                file,
                (exportJson: { sheets?: unknown[] }) => {
                    if (cancelled) return;
                    if (exportJson?.sheets?.length) {
                        const luckySheets = exportJson.sheets as LuckyExcelSheet[];
                        applyMergeCells(luckySheets);
                        applyExcelTextOverflow(luckySheets);
                        setSheets(luckySheets as unknown as Sheet[]);
                    } else {
                        setError("This spreadsheet could not be displayed.");
                    }
                },

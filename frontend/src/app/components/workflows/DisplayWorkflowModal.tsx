...
import type { DocketDocument, DocketWorkflow } from "../shared/types";
import { createTabularReview } from "@/app/lib/docketApi";
import { buildWorkflowLaunchMessage } from "@/app/lib/workflowLaunch";
...
// ---------------------------------------------------------------------------
function TabularPanel({ workflow }: { workflow: DocketWorkflow }) {
...
export function DisplayWorkflowModal({ workflows, workflow, onClose }: Props) {
    ...
    const [selected, setSelected] = useState<DocketWorkflow | null>(workflow);
    ...
    async function handleStartChat() {
        ...
        try {
            ...
            setNewChatMessages([
                buildWorkflowLaunchMessage(wf, assistantPrompt, files),
            ]);
            handleClose();
            router.push(
                projectId
    ...
    const workflowQuery = listSearch.toLowerCase().trim();
                            ...
                            <div className="w-80 shrink-0 flex flex-col border-t border-gray-200">
                                ...
                                <div className="px-3 py-2 shrink-0 border-b border-gray-100">
                                    <div className="flex items-center gap-1.5 rounded-md border border-gray-200 bg-gray-50 px-2.5 py-1">
                                        ...
                                        />
                                        {listSearch && (
                                            <button type="button" aria-label="Clear workflow search" onClick={() => setListSearch("")} className="text-gray-400 hover:text-gray-600">
                                                <X className="h-3 w-3" />
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className="overflow-y-auto flex-1">
                                    {workflows
                                        .filter((wfItem) =>
                                            !workflowQuery ||
                                            [wfItem.title, wfItem.description, wfItem.practice].some(
                                                (value) => value?.toLowerCase().includes(workflowQuery),
                                            ),
                                        )
                                        .map((wfItem) => {
                    ...
                    <>
                        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                            ...
                            <div className="px-5 pb-3 shrink-0">
                                ...
                                <label htmlFor="workflow-launch-message" className="block text-xs font-medium text-gray-700 mb-2">
                                    Message (optional)
                                </label>
                                <textarea
                                    id="workflow-launch-message"
                                    rows={3}

...
import type { DocketDocument, DocketProject, DocketWorkflow } from "../shared/types";
import {
    getProject,
    listProjects,
...
export function AddNewTRModal({
    ...
    const [projectDocs, setProjectDocs] = useState<DocketDocument[]>([]);
    const [loadingDocs, setLoadingDocs] = useState(false);

    // Full directory (when underProject is false)
    const [standaloneDocs, setStandaloneDocs] = useState<DocketDocument[]>([]);
    const [directoryProjects, setDirectoryProjects] = useState<DocketProject[]>(
        [],
    );
    ...
    const directoryFolders = isProjectMode
        ...
        : underProject
          ? []
          : directoryProjects;
    const flatProjectDocs: DocketDocument[] =
